...
// Jingkai: removed IIFE here, we don't need it behind a module system
...
function QRCodeModel(typeNumber, errorCorrectLevel) {
    ...
    this.modules = null;
    this.positionProbePatterns = null;
    this.moduleCount = 0;
    this.dataCache = null;
    this.dataList = [];
}
...
QRCodeModel.prototype = {
    ...
    isPositionProbe: function(row, col) {
        if (row < 0 || this.moduleCount <= row || col < 0 || this.moduleCount <= col) {
            throw new Error(row + "," + col);
        }

        if (!this.positionProbePatterns) {
            this.positionProbePatterns = this.getPositionProbePatterns();
        }

        return this.positionProbePatterns[row][col];
    },
    ...
    makeImpl: function(test, maskPattern) {
        ...
        this.modules = this.getPositionProbePatterns();
    ...
    generateModules() {
        var modules = new Array(this.moduleCount);
    ...
    getPositionProbePatterns: function() {
        var modules = this.generateModules();
        this.setupPositionProbePattern(0, 0, modules);
        this.setupPositionProbePattern(this.moduleCount - 7, 0, modules);
        this.setupPositionProbePattern(0, this.moduleCount - 7, modules);
        return modules;
    },
    ...
    Drawing.prototype.draw = function(oQRCode) {
        ...
        for (var row = 0; row < nCount; row++) {
            for (var col = 0; col < nCount; col++) {
                ...
                var bIsPositionProbe = oQRCode.isPositionProbe(row, col);
